...
// ── Outil Récap Mensuel ──────────────────────────────────────────────────────
function RecapTool({ historyData, ratingScale, userAvatar, onBack }) {
  ...
  const downloadSlide = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Erreur lors de la génération. Essaie de recharger la page.");
    }
      ...
      <div className="px-6 py-5 flex flex-col gap-5">
        ...
        <div className="flex overflow-x-auto gap-3 scrollbar-hide snap-x">
          {availableMonthsRaw.map((m) => {
            ...
            return (
              <button key={m} onClick={() => setSelectedMonth(m)} className={`snap-start flex-shrink-0 w-[4.5rem] h-[3.5rem] rounded-2xl border flex flex-col items-center justify-center transition-all ${isActive ? 'bg-[var(--color-primary-muted)] border-[var(--color-primary)]' : 'bg-white/5 border-white/10 opacity-60'}`}>
        ...
        <div className="flex items-center gap-1.5 px-2">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <div key={i} onClick={() => setCurrentSlide(i)} className={`h-1.5 flex-1 rounded-full cursor-pointer transition-all ${i === currentSlide ? 'bg-[var(--color-primary)] scale-y-150' : i < currentSlide ? 'bg-[var(--color-primary)]/40' : 'bg-white/10'}`} />
          ))}
        </div>
      ...
      {totalFilms > 0 ? (
        <div className="px-6 mb-6">
          <div className="rw-post-card relative shadow-[0_10px_40px_rgba(0,0,0,0.8)] border border-white/10">
            {currentSlide > 0 && (
              <button onClick={() => setCurrentSlide((s) => s - 1)} className="absolute left-2 top-1/2 -translate-y-1/2 z-50 w-8 h-8 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center shadow-lg">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6" /></svg>
              </button>
            )}
            {currentSlide < 5 && (
              <button onClick={() => setCurrentSlide((s) => s + 1)} className="absolute right-2 top-1/2 -translate-y-1/2 z-50 w-8 h-8 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center shadow-lg">
            ...
            <div className="rw-carousel-wrapper">
              <div className="rw-slide-track" style={{ transform: `translateX(-${currentSlide * (100 / 6)}%)` }}>
                ...
                <div className="rw-slide" id="rw-slide-2">
                  ...
                  <div className="absolute inset-0 z-2" style={{ background: 'linear-gradient(180deg,rgba(245,242,236,0.08) 0%,rgba(245,242,236,0.02) 25%,rgba(245,242,236,0.18) 45%,rgba(245,242,236,0.78) 62%,rgba(245,242,236,0.97) 78%,rgba(245,242,236,1.00) 88%)' }} />
                  <div className="absolute inset-0 right-[22px] z-10 flex flex-col justify-between px-[16px] pb-[18px]">
                    <div className="rw-top-row">
                      <div className="rw-pill-light"><span>{monthLabel}</span></div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-[6px] mt-[10px]">
                        {monthData.slice(0, 5).map((f, i) => (
                          <div key={i} className={`inline-flex items-center gap-[5px] bg-[#1E1E1E]/80 border border-[#1E1E1E]/10 rounded-[20px] px-[8px] py-[3px] font-sans text-[8px] font-medium text-white/90 whitespace-nowrap max-w-[250px] overflow-hidden text-ellipsis ${f.capucine ? 'border-[#8b1a3a]/35' : ''}`}>
                  ...
                  <div className="absolute top-[60px] bottom-0 left-0 right-[22px] flex flex-col px-[16px] pb-[14px] z-10 overflow-hidden">
                    <div className="flex-none flex items-start justify-between pt-[6px] border-b border-[#1E1E1E]/10 pb-[10px] mb-[10px]">
                      <div>
                        ...
                        <div className="font-syne font-extrabold text-[65px] leading-[0.88] text-[#1E1E1E] tracking-[-4px] flex items-baseline">
                          {avgNote.toFixed(1)}<span className="text-[28px] font-normal tracking-[-0.5px] text-[#E8B200] ml-1">/ {ratingScale}</span>
                        </div>
                      </div>
                      {highStarCount > 0 && (
                    ...
                    <div className="flex-none border-b border-[#1E1E1E]/10 pb-[10px] mb-[10px]">
                      ...
                      <div className="font-syne font-extrabold text-[72px] leading-[0.88] text-[#1E1E1E] tracking-[-3px] flex items-baseline">
                ...
                <div className="rw-slide" id="rw-slide-4" style={{ background: 'linear-gradient(180deg,#0D0D0D 0%,#111 100%)' }}>
                  ...
                  <div className="absolute inset-0 right-[22px] z-10 flex flex-col pt-[12px] px-[16px] pb-0">
                    <div className="rw-top-row !relative !p-0 mb-[8px] flex-shrink-0">
                      <div className="rw-pill-dark"><span>{monthLabel}</span></div>
                      <img className="rw-logo" src={userAvatar} alt="" crossOrigin="anonymous" />
                    </div>
                ...
                <div className="rw-slide" id="rw-slide-5">
                  <div className="absolute left-0 right-0 w-full h-[50%] overflow-hidden top-0">
                    {bestMovie?.affiche && <div className="absolute inset-0 bg-cover bg-center z-0 saturate-80 brightness-[0.55]" style={{ backgroundImage: `url(${bestMovie.affiche})` }} />}
                    ...
                    <div className="absolute inset-0 z-10 flex flex-col justify-end p-[10px_16px_12px]">
                      ...
                      <div className="flex items-center gap-[3px] text-[#E8B200]">
                        <span className="text-xs font-black mr-1">{bestMovie?.note}</span>
                        <svg className="w-[13px] h-[13px] fill-current" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="absolute left-0 right-0 w-full h-[50%] overflow-hidden bottom-0">
                    {worstMovie?.affiche && <div className="absolute inset-0 bg-cover bg-center z-0 brightness-[0.75] saturate-50" style={{ backgroundImage: `url(${worstMovie.affiche})` }} />}
                    <div className="absolute inset-0 z-1" style={{ background: 'linear-gradient(180deg,rgba(245,242,236,0.2) 0%,rgba(245,242,236,0.05) 25%,rgba(245,242,236,0.65) 65%,rgba(245,242,236,0.92) 100%)' }} />
                    <div className="absolute inset-0 z-10 flex flex-col justify-end p-[10px_16px_12px]">
                      ...
                      <div className="flex items-center gap-[3px] text-[#1E1E1E]/40">
                        <span className="text-xs font-black mr-1">{worstMovie?.note}</span>
                        <svg className="w-[13px] h-[13px] fill-current" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
                      </div>
                    </div>
                  </div>
                  <div className="absolute top-0 left-0 right-0 h-[58px] flex items-center justify-between px-[16px] z-30">
                    <div className="rw-pill-dark !bg-black/35 !border-white/10"><span>{monthLabel}</span></div>
                    <img className="rw-logo" src={userAvatar} alt="" crossOrigin="anonymous" />
                  </div>
                </div>
          ...
          <div className="mt-8 flex justify-center px-6">
            <button
              ...
              className="w-full max-w-sm h-14 rounded-2xl bg-[var(--color-primary)] text-black font-syne font-black text-sm tracking-wide flex items-center justify-center gap-3 shadow-[0_4px_20px_var(--color-primary-muted)] active:scale-95 transition-all disabled:opacity-50"
            >
              {isDownloading ? (
                <svg className="w-5 h-5 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeOpacity="0.25" /><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4" /></svg>
              ) : (
